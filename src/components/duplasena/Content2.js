...
export default class Content2 extends Component {
  ...
  componentDidMount() {
    axios.get('https://www.lotodicas.com.br/api/dupla-sena')
      .then((response) => {
        this.setState({
  ...
  render() {
    ...
    const sorteio = this.state.sorteio.join('/').replace(/,/g, '   ').split('/');
    ...
    return (
      <View style={styles.content1}>
        ...
        <View style={styles.content2Numbers}>
          <Text style={[styles.content2NText, styles.content2NText1]}>{sorteio[0]}</Text>
          <Text style={styles.content2NText}>{sorteio[1]}</Text>
        </View>
